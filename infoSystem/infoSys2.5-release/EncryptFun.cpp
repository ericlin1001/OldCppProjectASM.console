...
bool encryptFile(const char  *srcfile,const char  *destfile,const char *key,int keysize){
	...
	if(keysize==0){if(strcmp(srcfile,destfile)!=0)return copyFile(srcfile,destfile);return true;}
	...
	char *buffer=new char[keysize+1];//add 1 just to avoid error when keysize==1.
	int readlen=0;
...
bool encryptFile(const char  *srcfile,const char  *destfile){
	return encryptFile(srcfile,destfile,DEFAULT_KEY,strlen(DEFAULT_KEY));
}
bool decryptFile(const char  *srcfile,const char  *destfile){
	return decryptFile(srcfile,destfile,DEFAULT_KEY,strlen(DEFAULT_KEY));
}
