...
void help(){
	cout<<"all order:\nconnect,register,login,logoff,getUsers,myprofile,send,sendTo,exit."<<endl;
}
...
int main(){
	...
	while(1){
		...
		if(strcmp(buffer,"login")==0||strcmp(buffer,"l")==0){
			...
			c.login(user,pass);
		}else  if(strcmp(buffer,"register")==0||strcmp(buffer,"r")==0){
			char user[MAX_LEN_USERNAME];
			char pass[MAX_LEN_PASS];
			if(!c.isConn()){
				cout<<"connectting,please wait..."<<endl;
			...
			cout<<"user:";getStr(cin,user,MAX_LEN_USERNAME);
			cout<<"password:";getStr(cin,pass,MAX_LEN_PASS);
			if(c.registerUser(user,pass)==0){
				cout<<"Register successfully!"<<endl;
				cout<<"Please login in with this account."<<endl;
			}else{
			...
			vector<string>user=c.getUserList();
			cout<<"List:"<<endl;
			...
			for( i=0;i<user.size();i++){
				c.getUserDetail(user[i].c_str(),id_buffer);
				if(id_buffer!=INVALID_ID){
					ids.push_back(id_buffer);
				}
			}
			//
			...
			cin.get();
			Sleep(300);
		}else if(strcmp(buffer,"sendto")==0){
